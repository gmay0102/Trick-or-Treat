...
public sealed class PlayerController : Component
{
	...
	[Property] public GameObject SpectatorControllerPrefab {  get; set; }
	[Property] public GameObject MeatPrefab { get; set; }
	[Property] private GameObject AxeObject { get; set; }
	...
	[Sync] public float Height { get; private set; } = 72f;
	[Sync] private float HeightGoal { get; set; } = 72f;
	private BBox BoundingBox => new BBox( new Vector3( -Radius * WorldScale.x, -Radius * WorldScale.y, 0f ), new Vector3( Radius * WorldScale.x, Radius * WorldScale.y, HeightGoal * WorldScale.z ) );
	...
	private void Move(bool step) {
        ...
        if (step && IsOnGround)
        {
            characterControllerHelper.TryMoveWithStep(Time.Delta, 18f * WorldScale.z);
        }
        else
        {
	...
	private void CategorizePosition() {
        ...
        { // for some reason this fixes sliding down slopes when standing still, idek
            WorldPosition = sceneTraceResult.EndPosition + sceneTraceResult.Normal * 0f; 
        }
    }

    private SceneTrace BuildTrace(Vector3 from, Vector3 to) {
        return BuildTrace(base.Scene.Trace.Ray(in from, in to)).WithoutTags( "nocollide" );
    }
    ...
    private void GatherInput() {
		...
		if ( Input.Pressed( "Slot1" ) && (Connection.Local == Connection.Host) ) OnDeath();
		if ( Input.Pressed( "jump" ) ) Sandbox.Services.Achievements.Unlock( "youcantjump" );
    }

	public void OnDeath()
	{
		var owner = Network.Owner; //get the current client

		var ps = GameObject.GetComponent<PlayerState>();
		if ( ps.HasAxe )
		{
			ps.DropItem();
		}

		var spectator = SpectatorControllerPrefab.Clone( Camera.WorldPosition ); //set up da clone
		var meatpile = MeatPrefab.Clone(WorldPosition);

		

		spectator.Components.Get<SpectatorController>().LookAngle = LookAngle;
		meatpile.WorldPosition = WorldPosition;
		
		spectator.NetworkSpawn( owner ); //spawn da clone for da client
		meatpile.NetworkSpawn( owner );

		GameObject.Destroy(); //kill da player
	}
	...
	private void UpdateCitizenAnims() {
		...
		animationHelper.HoldType = HasAxe ? CitizenAnimationHelper.HoldTypes.Swing : CitizenAnimationHelper.HoldTypes.None;
		AxeObject.Enabled = (HasAxe);
	}
	...
	protected override void OnFixedUpdate() {
        ...
        if (IsCrouching) {
            ...
            var endPos = WorldPosition + new Vector3(0, 0, StandingHeight * WorldScale.z);
            var crouchTrace = Scene.Trace.Ray(startPos, endPos)
                                        .IgnoreGameObject(GameObject)
										.WithoutTags( "nocollide" )
                                        .Size(new BBox(new Vector3(-Radius, -Radius, 0f), new Vector3(Radius * WorldScale.x, Radius * WorldScale.y, 0)))
                                        .Run();
            if (crouchTrace.Hit) {
                HeightGoal = CroucingHeight;
                IsCrouching = true;
        ...
        if (AlreadyGrounded != IsOnGround) {
            if (IsOnGround) {
                var heightMult = Math.Abs(jumpHighestHeight - WorldPosition.z) / 46f;
                Stamina -= Stamina * StaminaLandingCost * 2.9625f * heightMult.Clamp(0, 1f);
                Stamina = (Stamina * 10).FloorToInt() * 0.1f;
                if (Stamina < 0) Stamina = 0;
            } else {
                jumpStartHeight = WorldPosition.z;
                jumpHighestHeight = WorldPosition.z;
            }
        } else {
        ...
        if (HeightDiff > 0f) WorldPosition += new Vector3(0, 0, HeightDiff * 0.5f);
        Velocity *= WorldScale;
        Move();
        CategorizePosition();
        Velocity /= WorldScale;
        ...
        if (jumpHighestHeight < WorldPosition.z) jumpHighestHeight = WorldPosition.z;
    }
	...
	protected override void OnUpdate() {
		...
		Camera.WorldPosition = WorldPosition + new Vector3(0, 0, Height * 0.89f * WorldScale.z);
